...
export default function Navbar() {
  ...
  return (
    <>
      {/* Optimized rendering: reduced backdrop blur intensity and dropped extreme saturation filters to maintain solid 60FPS scroll. added will-change transforms. */}
      <nav className="fixed top-3 left-1/2 -translate-x-1/2 w-[95%] max-w-[1400px] z-[60] flex items-center justify-between px-6 md:px-8 py-4 bg-[#050b14]/60 backdrop-blur-lg border border-white/[0.08] shadow-2xl rounded-2xl transition-all transform-gpu">
        <div className="flex items-center z-50">
          <Link href="/" onClick={() => setIsOpen(false)} className="flex items-center gap-2 transition-transform duration-300 hover:scale-[1.03] hover:drop-shadow-[0_0_15px_rgba(0,229,255,0.8)]">
            <img src="/Yudru_Logo.png" alt="Yudru Drone Solutions Logo" className="h-[45px] md:h-[65px] w-auto object-contain drop-shadow-md" />
          </Link>
        </div>
                  ...
                  return (
                    ...
                    >
                      <Link
                        href={item.href}
                        onClick={() => setIsOpen(false)}
                        className={`block w-full text-left py-4 px-5 text-[15px] tracking-widest font-bold uppercase rounded-r-xl transition-all duration-300 ${isActive
